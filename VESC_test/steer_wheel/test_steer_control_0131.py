            ...
            if joystick:
                ...
                if joy_cmd["emergency_stop"]:
                    ...
                    if abs(spin_cmd) > 0.01:
                        target_speed = MAX_SPEED * abs(joy_cmd["speed"]) if abs(joy_cmd["speed"]) > 0.01 else 0.5 
                        
                        if spin_cmd > 0:
                            controller.spin_clockwise(target_speed)
                        else:
                            controller.spin_counter_clockwise(target_speed)
